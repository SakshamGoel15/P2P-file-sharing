    ...
    public void run() {
      while (Complete_Peer_Files < Map_Peer.size()) {
        ...
        if (Interested_Msg.size() > 0) {
          ...
          try {
        	BufferedWriter writer = new BufferedWriter(new FileWriter(File_Log.getAbsolutePath(), true));
            Helper_Functions.Log(writer, Current_Node.Get_Peer_ID(), Connection_Random.Get_Peer_ID(), "change Unchoked Optimistically Neighbor");
            writer.close();
          } catch (IOException e1) {
...
class Wrong_Packet_Exception extends Exception {
	public Wrong_Packet_Exception(String c) {
		super(c);
	}
}